	...
	for(var i = 0 ; i < originData.fromNetworkmapd.length; i += 1) {
		...
		if(typeof thisClient[3] != "undefined") {
			if(ouiClientList.search(thisClient[3]) == -1) {
					oui_query_set_cookie(thisClient[3]);
			}
		}
	}
}
...
function clientInternetState(mac) {
	...
	for(var i = 0; i < timeSchedulingArray.length; i += 1) {
		if(!isNaN(parseInt(timeSchedulingArray[i]))) {
			...
			if(week_start == 0 && week_end == 0 && hour_start == 0 && hour_end == 0) { //all time setting
				internetState = true;
				break;
			}
...
function popClientListEditTable(mac, obj, name, ip, callBack) {
	...
	code += '<span title="Change client icon" style="margin-left:10px;" onclick="card_show_custom_image();">Change</span>';//untranslated
	code += '</div>';
	code += '</td>';
			...
			if(clientListIconArray[colIndex + (rowIndex * eachColCount)] != undefined) {
				...
				if(usericon_support) {
					if(document.getElementById("tdCardUserIcon") == null) {
						...
						cell.innerHTML = '<div id="divCardUserIcon" class="client_upload_div">Upload' +
	...
	document.getElementById("divCardUserIcon").style.display = "none";
	//1.check rc_support
	if(usericon_support) {

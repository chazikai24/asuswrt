...
function edit_confirm(){
	if(validForm()){
		...
		if(document.list_form.MULTIFILTER_MAC.value == MULTIFILTER_MAC_orig && document.list_form.MULTIFILTER_ENABLE.value == MULTIFILTER_ENABLE_orig && 
			...
			if(document.list_form.action_script.value == "restart_dnsmasq") {
		...
		if(document.list_form.flag.value == "background"){
			...
			setTimeout(function(){
				if(timeSchedulingFlag) { //if the latest internetMode is not time mode, then redirect to ParentalControl
...
function show_custom_image() {
	...
	if(display_state == "none") {
		...
		document.getElementById("changeIconTitle").innerHTML = "<#CTL_close#>";/*untranslated*/
	}
	else {
...
function oui_query(mac){
	...
	$.ajax({
		...
		type: 'GET',
		error: function(xhr) {
			if(document.getElementById("edit_client_block").style.display == "none")
				return true;
			else
				oui_query(mac);
		},
					...
					<table border="1" align="center" cellpadding="4" cellspacing="0" style="width:100%">
						...
						<tr>
							...
							<td class="client_icon_list_td">
								<div id="divUserIcon" class="client_upload_div">Upload<!--untranslated-->

...
int init_nvram(void)
{
...
#if defined(RTCONFIG_BCMARM) && !defined(RTCONFIG_BCM7) && !defined(RTCONFIG_BCM_7114) && !defined(RTCONFIG_CFEZ)
	char sdram_config[8];

#if defined(RTCONFIG_TCODE) && !defined(RTCONFIG_CFEZ)
	refresh_cfe_nvram();
#endif
	...
	if (strcmp(sdram_config, nvram_safe_get("sdram_config")))
	{
		nvram_commit();
		reboot(RB_AUTOBOOT);
	}
	...
	nvram_set("dsllog_interleavedepth", "");//interleave depth
...
chk_etfa()	/* after insmod et */
{
	if (FA_ON(fa_mode)) {
		if (
			...
			!nvram_get_int("PA") ||
#endif
#if (!defined(RTCONFIG_BCM7) && !defined(RTCONFIG_BCM7114))
#ifdef RTCONFIG_RGMII_BCM_FA
			(get_fa_rev()<0) ||
#else
			(get_fa_rev()<3) ||
